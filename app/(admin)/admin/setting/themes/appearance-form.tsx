...
const appearanceFormSchema = z.object({
  ...
  themeConfig: z.object({
    ...
    'button-background-color-hover': z.string().min(4).max(9).regex(/^#/).optional(),
...
type ThemeConfigKeys = keyof typeof appearanceFormSchema.shape.themeConfig.shape;
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem className="space-y-1">
          ...
          <FormDescription>選擇您的喜愛的主題，搭配您的個人風格</FormDescription>
          <FormMessage />

          <RadioGroup
            onValueChange={field.onChange}
            defaultValue={String(field.value)}
            className="grid max-w-md grid-cols-2 sm:grid-cols-3 gap-3 md:gap-5 pt-2"
                ...
                <FormItem key={id} className="flex cursor-pointer">
                  <FormLabel className="[&:has([data-state=checked])>div]:border-primary">
                    ...
                    <div className="select-none items-center rounded-md border-2 border-muted p-1 hover:border-accent">
                      <Image src={preview} alt={name} width={150} height={300} />
                    </div>
                    <span className="block w-full p-2 text-center font-normal">{name}</span>
                  </FormLabel>
                </FormItem>
              );
            })}
...
export function ThemeConfigForm({ form }: { form: UseFormReturn<z.infer<typeof appearanceFormSchema>> }) {
      ...
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="item-1">
          ...
          <AccordionContent className="grid gap-3">
            ...
            />
            {values.isGradient && (
              <ColorForm form={form} label={'上層顏色'} name={'themeConfig.background-color-2' as any} />
            )}
            <ColorForm
              form={form}
        ...
        <AccordionItem value="item-2">
          ...
          <AccordionContent className="grid gap-3">
            <ColorForm form={form} label={'背景顏色'} name={'themeConfig.bio-background-color' as any} />
            <ColorForm form={form} label={'文字顏色'} name={'themeConfig.text-color' as any} />
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="item-3">
          <AccordionTrigger>按鈕設定</AccordionTrigger>
          <AccordionContent className="grid gap-3">
            <ColorForm form={form} label={'背景顏色'} name={'themeConfig.button-background-color' as any} />
            <ColorForm form={form} label={'懸浮時背景顏色'} name={'themeConfig.button-background-color-hover' as any} />
            <ColorForm form={form} label={'邊框顏色'} name={'themeConfig.button-border-color' as any} />
            <ColorForm form={form} label={'文字顏色'} name={'themeConfig.button-text-color' as any} />
            <ColorForm form={form} label={'懸浮文字顏色'} name={'themeConfig.button-text-color-hover' as any} />
            <SliderFormWithPx
              form={form}
              ...
              step={1}
            />
            <SliderFormWithPx form={form} name={'themeConfig.button-radius'} label={'圓角'} min={0} max={50} step={1} />
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </>
...
export function AppearanceForm() {
  ...
  const defaultValues: Partial<AppearanceFormValues> = {
    ...
    isGradient: user?.theme?.settings
      ? Boolean(user?.theme?.settings['background-color'] !== user?.theme?.settings['background-color-2'])
      : false,
    ...
    const subscription = form.watch(async (data, { name, type }) => {
      if (type === 'change' && name === 'theme') {
        ...
        if (theme) {
          ...
          const isGradient = Boolean(theme.settings['background-color'] !== theme.settings['background-color-2']);
      ...
      } else if (type === 'change' && (name?.includes('themeConfig') || name === 'isGradient')) {
        const settings = data.themeConfig;
        if (!data.isGradient && settings?.['background-color']) {
          form.setValue('themeConfig.background-color-2', settings?.['background-color'] ?? '');
        }
        const theme = templates.find(({ id }) => id === Number(data.theme));
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className={cn('sm:col-span-3 space-y-8', { hidden: isPreview })}>
        ...
        <button
          ...
          className={cn(
            'fixed z-50 right-2 bottom-2 items-center grid place-items-center rounded-full border h-12 w-12 text-sm font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-primary text-primary-foreground hover:bg-primary/80',
            { 'sm:hidden': !isPreview }
          )}
        >
          {isPreview ? '關閉' : '預覽'}
